...
class GNNBenchmark(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None):
        self.name = name
        name2files = {'coautorcs': 'ms_academic_cs.npz',
                      'coautorphys': 'ms_academic_phy.npz',
...
class SparseGraph:
    ...
    def __init__(self, adj_matrix, attr_matrix=None, labels=None,
        ...
        attr_matrix : sp.csr_matrix or np.ndarray, shape [num_nodes, num_attr],
            optional
        ...
        if sp.isspmatrix(adj_matrix):
            ...
            raise ValueError("Adjacency matrix must be in sparse format "
                             "(got {0} instead)"
        ...
        if attr_matrix is not None:
            if sp.isspmatrix(attr_matrix):
                ...
                raise ValueError("Attribute matrix must be a sp.spmatrix or a "
                                 "np.ndarray (got {0} instead)"
                                 .format(type(attr_matrix)))

            if attr_matrix.shape[0] != adj_matrix.shape[0]:
                raise ValueError("Dimensions of the adjacency and attribute """
                                 "matrices don't agree")

        if labels is not None:
            if labels.shape[0] != adj_matrix.shape[0]:
                raise ValueError("Dimensions of the adjacency matrix and the "
                                 "label vector don't agree")

        if node_names is not None:
            if len(node_names) != adj_matrix.shape[0]:
                raise ValueError("Dimensions of the adjacency matrix and the "
                                 "node names don't agree")

        if attr_names is not None:
            if len(attr_names) != attr_matrix.shape[1]:
                raise ValueError("Dimensions of the attribute matrix and the "
                                 "attribute names don't agree")
    ...
    def is_directed(self):
        """Check if the graph is directed (adjacency matrix not symmetric)"""
...
def load_npz_to_sparse_graph(file_name):
    ...
    with np.load(file_name) as loader:
        ...
        if 'labels_data' in loader:
            ...
            labels = sp.csr_matrix((loader['labels_data'],
    ...
    return SparseGraph(adj_matrix, attr_matrix, labels, node_names, attr_names,
